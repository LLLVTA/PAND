...
class TeacherStudent(nn.Module):
    def __init__(self, teacher, student, data_attributes, use_teacher=True):
        ...
        if use_teacher:
            ...
            # Check if CoOp text features should be loaded
            if hasattr(teacher, 'coop_text_features') and teacher.coop_text_features:
                self.frozen_nlp_features = self.load_coop_text_features(teacher.coop_text_features)
                print(f"[Stage B] Loaded CoOp text features from: {teacher.coop_text_features}")
            else:
                self.frozen_nlp_features = self.get_frozen_nlp_features(data_attributes)
                print(f"[Stage B] Using default CLIP text features with prompt: {data_attributes.prompt_tmpl}")
    
    def load_coop_text_features(self, coop_features_path):
        """Load pre-extracted CoOp text features from .pt file"""
        features_data = torch.load(coop_features_path)
        text_features = features_data['text_features']  # [n_cls, dim], already normalized
        
        # Validate class count
        expected_classes = self.data_attributes.class_num
        if text_features.shape[0] != expected_classes:
            raise ValueError(
                f"CoOp features class count mismatch: "
                f"got {text_features.shape[0]}, expected {expected_classes}"
            )
        
        print(f"  ✓ CoOp features shape: {text_features.shape}")
        print(f"  ✓ Classes: {expected_classes}")
        print(f"  ✓ Context tokens: {features_data['n_ctx']}")
        
        return text_features.detach()  # Return as detached tensor
